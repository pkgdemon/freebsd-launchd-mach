/*
 * launch.h — local copy of Apple's <launch.h> for libxpc.
 *
 * libxpc's xpc/xpc.h pulls in <launch.h> for the launch_data_t /
 * launch_data_type_t types used by ld2xpc() in xpc_misc.c. On FreeBSD
 * we don't (yet) install a real /usr/include/launch.h, so libxpc
 * carries its own copy. Vendored verbatim from ravynOS's
 * lib/liblaunch/launch.h — declarations only; the actual launch_data_*
 * / launch_msg() implementations live in liblaunch (not yet ported),
 * which is fine because libxpc itself only consumes the enum + typedef
 * and never calls those functions.
 *
 * Reachable via the Makefile's -I${.CURDIR}, same pattern as
 * Availability.h and uuid/uuid.h.
 */
