...
#if APPLE_ICU_CHANGES
// rdar://121488288 (21E188: footprint regressions in multiple processes since 21E187)
LSR LikelySubtags::makeMaximizedLsrFrom(const Locale &locale, UErrorCode &errorCode) const {
...
// rdar://57899428 commit a429a8a3da.. Integrate open-source ICU 66 preview...; ...fix or handle all remaining test failures
    if (language.compare("und") == 0 || language.compare("root") == 0) { // Apple adds root
#else
...
// the "Aran" script code to likelySubtag.xml.  This is temporary code that should be removed when ICU-22571 is fixed.
    if (script.compare("Aran") == 0) {
        if (language.empty() && region.empty()) {
            return LSR("ur", "Aran", "PK", LSR::EXPLICIT_LSR, errorCode);
        } else if (region.empty()) {
            if (language.compare("ks") == 0) {
                return LSR("ks", "Aran", "IN", LSR::EXPLICIT_LSR, errorCode);
            } else if (language.compare("ur") == 0 || language.compare("pa") == 0) {
                return LSR(language, "Aran", "PK", LSR::EXPLICIT_LSR, errorCode);
            }
        } else if (language.empty()) {
            if (region.compare("IN") == 0) {
                return LSR("ks", "Aran", "IN", LSR::EXPLICIT_LSR, errorCode);
            } else if (region.compare("PK") == 0) {
                return LSR("ur", "Aran", "PK", LSR::EXPLICIT_LSR, errorCode);
            }
        }
    } else if (script.empty()) {
        if (language.empty() && region.compare("PK") == 0) {
            return LSR("ur", "Aran", "PK", LSR::EXPLICIT_LSR, errorCode);
        } else if (language.compare("ur") == 0) {
            if (region.empty()) {
                return LSR("ur", "Aran", "PK", LSR::EXPLICIT_LSR, errorCode);
            } else {
                return LSR("ur", "Aran", region, LSR::EXPLICIT_LSR, errorCode);
            }
        } else if (language.compare("ks") == 0) {
            if (region.empty()) {
                return LSR("ks", "Aran", "IN", LSR::EXPLICIT_LSR, errorCode);
            } else {
                return LSR("ks", "Aran", region, LSR::EXPLICIT_LSR, errorCode);
            }
        } else if (language.compare("pa") == 0 && region.compare("PK") == 0) {
            return LSR("pa", "Aran", "PK", LSR::EXPLICIT_LSR, errorCode);
        }
    }
#endif // APPLE_ICU_CHANGES
