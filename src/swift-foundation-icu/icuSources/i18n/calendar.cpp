...
typedef enum ECalType {
    ...
    CALTYPE_HINDU_LUNAR,
    CALTYPE_HINDU_SOLAR,
    CALTYPE_INDIAN_LUNISOLAR_VIKRAM,
    CALTYPE_INDIAN_LUNISOLAR_GUJARATI,
    CALTYPE_INDIAN_LUNISOLAR_KANNADA,
...
static Calendar *createStandardCalendar(ECalType calType, const Locale &loc, UErrorCode& status) {
    ...
    switch (calType) {
        ...
        case CALTYPE_DANGI:
            ...
            break;
        case CALTYPE_HINDU_LUNAR:
            cal.adoptInsteadAndCheckErrorCode(new HinduLunarCalendar(loc, status), status);
            break;
        case CALTYPE_HINDU_SOLAR:
            cal.adoptInsteadAndCheckErrorCode(new HinduSolarCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_LUNISOLAR_VIKRAM:
            cal.adoptInsteadAndCheckErrorCode(new HinduLunisolarVikramCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_LUNISOLAR_GUJARATI:
            cal.adoptInsteadAndCheckErrorCode(new HinduLunisolarGujaratiCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_LUNISOLAR_KANNADA:
            cal.adoptInsteadAndCheckErrorCode(new HinduLunisolarKannadaCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_LUNISOLAR_MARATHI:
            cal.adoptInsteadAndCheckErrorCode(new HinduLunisolarMarathiCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_LUNISOLAR_TELUGU:
            cal.adoptInsteadAndCheckErrorCode(new HinduLunisolarTeluguCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_SOLAR_BANGLA:
            cal.adoptInsteadAndCheckErrorCode(new HinduSolarBanglaCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_SOLAR_TAMIL:
            cal.adoptInsteadAndCheckErrorCode(new HinduSolarTamilCalendar(loc, status), status);
            break;
        case CALTYPE_INDIAN_SOLAR_MALAYALAM:
            cal.adoptInsteadAndCheckErrorCode(new HinduSolarMalayalamCalendar(loc, status), status);
            break;
...
// Apple is using INT32_MAX / 2 as a sentinel value in some calculations--
// see Apple changes to EraRules::getStartYear()
    if (year > INT32_MAX / 2) {
#else
