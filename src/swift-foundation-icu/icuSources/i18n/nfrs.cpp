...
// rdar:/
NFRuleSet::parse(const UnicodeString& text, ParsePosition& pos, double upperBound, uint32_t nonNumericalExecutedRuleMask, int32_t recursionCount, Formattable& result, UBool lenient) const
#else
...
#if APPLE_ICU_CHANGES
// rdar:/
            UBool success = nonNumericalRules[i]->doParse(text, workingPos, 0, upperBound, nonNumericalExecutedRuleMask, recursionCount + 1, tempResult, lenient || isDecimalFormatRuleParseable() );
#else
