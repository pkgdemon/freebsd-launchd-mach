...
// The OSICU change to tighten error handling breaks the Apple logic for the last era boundary in Japanese--
// the old version of this function swallowed the error code instead of returning it to the caller.  To
// maintain compatibility, we swallow the error code if we're on the past-the-end era number.
    if (status == U_ILLEGAL_ARGUMENT_ERROR && era == gJapaneseEraRules->getNumberOfEras()) {
        status = U_ZERO_ERROR; // (the value returned by getStartYear() should still be good-- just swallow the error)
    }
...
// the old version of this function swallowed the error code instead of returning it to the caller.  To
// maintain compatibility, we swallow the error code if we're on the past-the-end era number.
    if (status == U_ILLEGAL_ARGUMENT_ERROR && internalGet(UCAL_ERA, gCurrentEra) == gJapaneseEraRules->getNumberOfEras()) {
        status = U_ZERO_ERROR; // (the value returned by getStartYear() should still be good-- just swallow the error)
    }
#endif // APPLE_ICU_CHANGES
    if (U_FAILURE(status)) {
