...
int32_t newYear(const icu::ChineseCalendar::Setting&, bool, int32_t, UErrorCode&);
#else
...
// The Apple changes to use a static table of new-year adjustments is incomparible with the OSICU 76
// changes to the Dangi calendar
    struct MonthInfo monthInfo = computeMonthInfo(setting, canUseAdjustmentTable, gyear, newMoon, status);
#else
...
// changes to the Dangi calendar
    ...
    if (days < theNewYear) {
        theNewYear = newYear(setting, canUseAdjustmentTable, gyear-1, status);
#else
    int32_t theNewYear = newYear(setting, gyear, status);
    if (U_FAILURE(status)) {
        return output;
...
#if APPLE_ICU_CHANGES
// rdar://17888673 688c98a2e1.. Speed up Calendar use of chinese: Refactor CalendarAstronomer to provide static.
    if (canUseAdjustmentTable && gyear >= 1900 && gyear <= 2100) {
        // Don't use cache, just return linear estimate + table correction
        int32_t gyearadj = gyear - 1900;
